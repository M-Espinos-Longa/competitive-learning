...
class DuelingDDQNModel(nn.Module):
    ...
    def model_init(self, num_obs_c, num_obs_h, num_obs_w, num_actions, num_hidden_units,
        ...
        """
        Model initialisation. Uses agents_info neural network data (described in
        DuelingDDQNAgents.py script)
        Inputs:
        ...
        self.linear_a = nn.Linear(num_input_linear_layer, num_hidden_units[0]).to(device)
        self.advantage = nn.Linear(num_hidden_units[0], num_actions).to(device)
    ...
    def forward_pass(self, input):
        ...
        Outputs:
            + qvals (tensor) -> state-action values (value + (advantages - mean(advantages)).
        ...
        # advantage MLP
        outlin_a = self.relu(self.linear_a(flat))
        adv = self.advantage(outlin_a)

        # output
        qvals = val + adv - torch.mean(adv, dim=1, keepdim=True)
