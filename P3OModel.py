...
class P3OModel(nn.Module):
    ...
    def forward_pass_actor(self, input):
        """
        Inputs:
            + input (tensor) -> batch of osbervations
        Outputs:
            + dist (tesnor) -> softmax policy distribution
        """
        ...
        dist = self.policy(outlin_p)

        return dist

    def forward_pass_critic(self, input):
        """
        Inputs:
            + input (tensor) -> batch of osbervations
        Outputs:
